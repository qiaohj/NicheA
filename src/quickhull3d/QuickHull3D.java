...
public class QuickHull3D {
	...
	public Face[] getFaceArray() {
		...
		Vertex[] vtxArray = new Vertex[this.numVertices];
		for (int i = 0; i < this.numVertices; i++) {
			vtxArray[i] = new Vertex();
			vtxArray[i].pnt = vertices[i];
			vtxArray[i].index = i;
		}
		int nFaces = this.faces.size();
		int[][] rawFaces = getFaces(0);
		Face[] allFaces = new Face[nFaces];
		for (int i = 0; i < nFaces; i++) {
			allFaces[i] = Face.create(vtxArray, rawFaces[i]);
		}
		return allFaces;
	}
	...
	public double getVolume() {
		double V = 0.0D;
		Point3d[] vertices = getVertices();
		Point3d apex = vertices[0];
		Face[] faces = getFaceArray();
		int nFaces = faces.length;
		for (int i = 0; i < nFaces; i++) {
			Face thisFace = faces[i];
			double H = Math.abs(thisFace.distanceToPlane(apex));
			double A = getArea(thisFace, vertices);
			V += A * H / 3.0D;
		}
		return V;
	}

	public double getArea(Face thisFace, Point3d[] vertices) {

		int fVerts = thisFace.numVerts;
		// System.out.println(fVerts);
		int[] idx = new int[fVerts];
		thisFace.getVertexIndices(idx);
		if (fVerts == 3) {
			double a = distance(vertices[idx[0]], vertices[idx[1]]);
			double b = distance(vertices[idx[1]], vertices[idx[2]]);
			double c = distance(vertices[idx[2]], vertices[idx[0]]);
			double p = (a + b + c) / 2;
			return Math.sqrt(p * (p - a) * (p - b) * (p - c));
		} else {
			double a = distance(vertices[idx[0]], vertices[idx[1]]);
			double b = distance(vertices[idx[1]], vertices[idx[2]]);
			double c = distance(vertices[idx[2]], vertices[idx[3]]);
			double d = distance(vertices[idx[3]], vertices[idx[0]]);
			double e = distance(vertices[idx[0]], vertices[idx[2]]);
			double f = distance(vertices[idx[1]], vertices[idx[3]]);
			return 0.25d * Math
					.sqrt(4
							* Math.pow(e, 2)
							* Math.pow(f, 2)
							- Math.pow(
									(Math.pow(b, 2) + Math.pow(d, 2)
											- Math.pow(a, 2) - Math.pow(c, 2)),
									2));
		}
	}

	private double distance(Point3d v1, Point3d v2) {
		return Math.sqrt(Math.pow((v1.x - v2.x), 2)
				+ Math.pow((v1.y - v2.y), 2) + Math.pow((v1.z - v2.z), 2));
	}
}
